          ...
          <div style={{ background: 'white', borderBottom: '1px solid #e8edf2', padding: '12px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 11, color: '#778899' }}>{isInstructor ? 'Instrutor' : 'Recrutador'}</div>
              </div>
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              {user?.role === 'ADMIN' && (
                <button onClick={() => navigate('admin')} style={{ background: '#fef2f2', border: 'none', borderRadius: 8, padding: '6px 10px', cursor: 'pointer', fontSize: 12, fontWeight: 700, color: '#b91c1c' }}>Admin</button>
              )}
              <button onClick={() => navigate('home')} style={{ background: '#f0f6ff', border: 'none', borderRadius: 8, padding: '6px 10px', cursor: 'pointer', fontSize: 12, color: '#1e4a8a' }}>Site</button>
