...
const w2h = [
  { tag: 'O QUÊ', color: '#7a3a8a', title: 'Gestão orientada a resultados', desc: 'Estruturamos metas, indicadores e programas de incentivo para que toda a equipe saiba exatamente o que precisa entregar e seja reconhecida por isso.' },
  { tag: 'POR QUÊ', color: '#8a4a9a', title: 'O que não é medido não é gerenciado', desc: 'Sem metas claras e KPIs bem definidos, a equipe trabalha sem direção. Performance sem gestão é sorte — não estratégia.' },
  { tag: 'PARA QUEM', color: '#9a5ab0', title: 'Varejistas que querem crescer com consistência', desc: 'Empresas que já têm uma equipe estruturada e querem extrair o máximo de cada colaborador com dados, metas e reconhecimento.' },
  { tag: 'COMO', color: '#aa6ac0', title: 'Diagnóstico → Metas → Acompanhamento', desc: 'Analisamos os resultados atuais, definimos OKRs e KPIs realistas, criamos rituais de gestão e implementamos dashboards para decisão ágil.' },
  { tag: 'QUANDO', color: '#ba7ad0', title: 'Primeiros resultados em 45 dias', desc: 'As metas e indicadores ficam prontos nas primeiras 2 semanas. O programa de incentivo é lançado no mês 1, com primeiros resultados medidos em 45 dias.' },
  { tag: 'QUANTO', color: '#8a4a9a', title: 'Proposta personalizada', desc: 'O investimento varia conforme o tamanho da equipe e os módulos contratados. Agende uma conversa gratuita e veja o que faz sentido para o seu negócio.' },
]
...
export default function PerformancePage({ navigate }) {
  ...
  return (
    <div className="fade-in">
      ...
      {/* ── Hero ── */}
      <div style={{ background: 'linear-gradient(135deg, #4a1a6e 0%, #7a3a9e 50%, #9a5ab8 100%)', padding: isMobile ? '52px 24px 48px' : '72px 40px 64px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 900, margin: '0 auto', position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
            <button onClick={() => navigate('home')} style={{ background: 'white', color: '#4a1a6e', border: 'none', borderRadius: 32, padding: '13px 28px', cursor: 'pointer', fontWeight: 700, fontSize: 15, boxShadow: '0 4px 16px rgba(0,0,0,0.15)' }}>Solicitar proposta</button>
      ...
      <div style={{ background: '#f4f7fb', padding: isMobile ? '48px 20px' : '64px 20px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: 3, color: '#7a3a9e', textTransform: 'uppercase', textAlign: 'center', marginBottom: 8 }}>Entenda o serviço</p>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3, 1fr)', gap: 20 }}>
            {w2h.map((item, i) => (
              <div key={i} style={{ background: 'white', borderRadius: 18, padding: '28px 24px', boxShadow: '0 4px 20px rgba(30,74,138,0.07)', borderTop: `4px solid ${item.color}`, transition: 'transform 0.2s, box-shadow 0.2s' }}
                onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-4px)'; e.currentTarget.style.boxShadow = '0 12px 32px rgba(74,26,110,0.12)' }}
      ...
      <div id="servicos-lista" style={{ background: 'white', padding: isMobile ? '48px 20px' : '64px 20px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: 3, color: '#7a3a9e', textTransform: 'uppercase', textAlign: 'center', marginBottom: 8 }}>O que está incluído</p>
          <h2 style={{ fontSize: isMobile ? 22 : 28, fontWeight: 800, color: '#1e3a6e', textAlign: 'center', marginBottom: 40 }}>Serviços do Pilar <span style={{ color: '#7a3a9e' }}>Performance</span></h2>
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3, 1fr)', gap: 16 }}>
            {services.map((s, i) => (
              <div key={i} onClick={() => setActiveService(activeService === i ? null : i)}
                style={{ background: activeService === i ? 'linear-gradient(135deg, #f4e8fc, #ead4f8)' : '#f8fafc', borderRadius: 14, padding: '20px', border: activeService === i ? '1.5px solid #7a3a9e' : '1.5px solid #e8edf4', cursor: 'pointer', transition: 'all 0.2s' }}
                onMouseEnter={e => { if (activeService !== i) e.currentTarget.style.borderColor = '#c090d8' }}
      ...
      {/* ── CTA ── */}
      <div style={{ background: 'linear-gradient(135deg, #4a1a6e, #7a3a9e)', padding: isMobile ? '48px 24px' : '64px 40px', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
          <button onClick={() => navigate('home')} style={{ background: 'white', color: '#4a1a6e', border: 'none', borderRadius: 32, padding: '14px 32px', cursor: 'pointer', fontWeight: 700, fontSize: 15 }}>Solicitar proposta gratuita</button>
