                    ...
                    <div key={item.label} ref={dropRef} style={{ position: 'relative' }}>
                      ...
                      >
                        ...
                        <span style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
                          {item.label}
                          <span style={{ fontSize: 8, display: 'inline-block', transition: 'transform 0.2s', transform: servicosOpen ? 'rotate(180deg)' : 'none' }}><ChevronDown size={10} /></span>
                        </span>
                      </button>
                      {servicosOpen && (
                          ...
                          {pilares.map((p) => (
                            ...
                            >
                              ...
                              <div>
                                ...
                                <div style={{ fontSize: 12.5, color: '#556677', marginTop: 1 }}>{p.desc}</div>
                              </div>
                              <span style={{ marginLeft: 'auto', fontSize: 12, color: '#778899' }}>›</span>
                            </div>
                          ))}
                        </div>
                      )}
              ...
              <div style={{ position: 'relative' }}>
                ...
                {userMenu && (
                  <div style={{ position: 'absolute', right: 0, top: 'calc(100% + 8px)', background: 'white', borderRadius: 12, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', border: '1px solid #e8edf2', minWidth: 170, overflow: 'hidden', zIndex: 100 }}>
                    {[
                      { icon: <User size={14} />, label: 'Meu Painel',   page: (user?.role === 'RECRUITER' || user?.role === 'INSTRUCTOR') ? 'dashboard-recrutador' : 'dashboard-candidato' },
                ...
                return (
                  <div key={item.label}>
                    ...
                    >
                      ...
                      <span style={{ fontSize: 10, color: '#4a9edd', transform: mobileServicosOpen ? 'rotate(180deg)' : 'none', display: 'inline-flex', transition: 'transform 0.2s' }}><ChevronDown size={14} /></span>
                    </button>
                    {mobileServicosOpen && (
